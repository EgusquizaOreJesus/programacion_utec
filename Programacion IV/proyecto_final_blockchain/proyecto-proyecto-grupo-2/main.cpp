...
void DrawLoading2() {
    ...
    Texture2D spritesheet = LoadTexture("../assets/sprite2_2.png");
    int spriteWidth = spritesheet.width /14;
    int spriteHeight = spritesheet.height;  // Si los sprites están en una sola columna
    Rectangle sourceRect = { 0, 0, static_cast<float>(spriteWidth), static_cast<float>(spriteHeight) };
    int currentFrame2 = 0;
    float frameTime = 0.0f;
    Vector2 position = { 800, 260 };
    while (!exitLoading) {
        frameTime += GetFrameTime();

        // Cambiar al siguiente frame cada 0.1 segundos
        if (frameTime >= 0.1f)
        {
            frameTime = 0.0f;
            currentFrame2++;

            // Si llegamos al último frame, volvemos al primero
            if (currentFrame2 > 14)
                currentFrame2 = 0;

            // Actualizamos el rectángulo de recorte
            sourceRect.x = currentFrame2 * spriteWidth;
        }
        BeginDrawing();
        ClearBackground(BLACK);
        DrawTextureRec(spritesheet, sourceRect, position, WHITE);
        DrawText(loadingText.c_str(), screenWidth / 2 - MeasureText(loadingText.c_str(), fontSize) / 2 -30, 550, fontSize, WHITE);
        ...
        DrawText(dots.c_str(), screenWidth / 2 + MeasureText(loadingText.c_str(), fontSize) / 2 -20, 550, fontSize, WHITE);
        ...
        if (isOption2 && IsKeyPressed(KEY_ENTER)) {
            ...
            if (value1.empty() || value2.empty()){
                ...
                if (selectedOptionH == 0){
                    ...
                    auto v = alfa.monto_range_search_monto(monto1_,monto2_);
                    DrawLoading2();

                    auto indices = alfa.get_block_id_from_monto(v);
                    if (!indices.empty()){
                        DrawBlocks_index(true,alfa,indices);
                    }
                }
...
void DrawAlterar(blockchain& alfa,bool& shouldExit, bool& shouldMenu){
    ...
    const int menuOptionsCountH = 3;
    int selectedOptionH = 0;
    std::string options2[menuOptionsCountH] = {"Emisor", "Receptor","Monto" };
    while (!exitDraw)
    {
        if (IsKeyPressed(KEY_LEFT)) {
            selectedOptionH = (selectedOptionH - 1 + menuOptionsCountH) % menuOptionsCountH;
        ...
        if (IsKeyPressed(KEY_ENTER))
        {
            ...
            if ( valorBuscado.empty() || indexBloqueBuscado.empty() || indexTransaccionBuscado.empty()){
                resultado = "Registro erroneo";
                startTime = GetTime(); // Reiniciar el tiempo de inicio cuando se muestra el mensaje
                ...
                std::transform(valorBuscado.begin(), valorBuscado.end(), valorBuscado.begin(), [](unsigned char c) {
                    return std::tolower(c);
                });
                ...
                else if (selectedOptionH == 2){
                    double monto = std::stod(valorBuscado);
                    cout << "monto:" << monto << endl;
                    cout << "bloque index" << bloqueindex << endl;
                    cout << "transaccion index" << transaccionindex << endl;
        ...
        if (selectedOptionH >= 0 && selectedOptionH < menuOptionsCountH) {
            DrawText("<", screenWidth / 2 - MeasureText(options2[selectedOptionH].c_str(), fontsize) - 100, 350, fontsize, GREEN);
            DrawText(options2[selectedOptionH].c_str(), screenWidth / 2 - MeasureText(options2[selectedOptionH].c_str(), fontsize) / 2, 350, fontsize, GREEN);
            DrawText(">", screenWidth / 2 + MeasureText(options2[selectedOptionH].c_str(), fontsize) + 100, 350, fontsize, GREEN);
        }
...
void DrawInsert(blockchain& alfa,bool& shouldExit, bool& shouldMenu){
    ...
    double startTime = GetTime(); // Tiempo de inicio del mensaje
    double intervalo = 2.0; // Duración del mensaje en segundos
    Texture2D image = LoadTexture("../assets/KON_copi.png");

    const int menuOptionsCountH = 2;
    int selectedOptionH = 0;
    std::string options2[menuOptionsCountH] = {"Individual", "Total" };
    vector<transaccion> transacciones;
    bool isTotal= false;
    while (!exitDraw)
    {
...
//                shouldMenu = true;
            ...
            if(selectedOptionH == 1){
                ...
                if (MontoBuscar.empty() || ReceptorBuscar.empty() || EmisorBuscar.empty() || fechaBuscar.empty()){
                    resultado = "Registro erroneo";
                    startTime = GetTime(); // Reiniciar el tiempo de inicio cuando se muestra el mensaje
                }
                else{
                    ...
                    transaccion temp(montoindex,EmisorBuscar,ReceptorBuscar,dia,mes,anio);
                    transacciones.push_back(temp);
                    for(auto a: transacciones){
                        a.display();
                    }
                }
                isTotal = true;
        ...
        if (IsKeyPressed(KEY_ESCAPE)) {
            if (isTotal){
                alfa.insert_block_with_transaction(transacciones);
            }
            shouldMenu = true;
            exitDraw = true;
        }
        ...
        if (selectedOptionH >= 0 && selectedOptionH < menuOptionsCountH) {
            DrawText("<", screenWidth / 2 - MeasureText(options2[selectedOptionH].c_str(), fontSize) - 100, 400, fontSize, GREEN);
            DrawText(options2[selectedOptionH].c_str(), screenWidth / 2 - MeasureText(options2[selectedOptionH].c_str(), fontSize) / 2, 400, fontSize, GREEN);
            DrawText(">", screenWidth / 2 + MeasureText(options2[selectedOptionH].c_str(), fontSize) + 100, 400, fontSize, GREEN);
        }
//        DrawText("Resultado:", 800, 620, 20, DARKGRAY);
//        DrawText(resultado.c_str(), 830, 625, 20, DARKGRAY);
...
void DrawSearch(blockchain& alfa,bool& shouldExit, bool& shouldMenu) {
    ...
    while (!enterKeyPressed) {
        ...
        DrawText("Search Options:", screenWidth / 2 - MeasureText("Search Options:", fontSize) / 2, screenHeight / 2 - (menuOptionsCount + 1) * fontSize + 70, fontSize, WHITE);
        ...
        for (int i = 0; i < menuOptionsCount; i++) {
            if (i == selectedOption) {
                DrawText("->", screenWidth / 2 - MeasureText("->", fontSize) - 10, screenHeight / 2 - (menuOptionsCount - i) * fontSize + 80, fontSize, GREEN);
                DrawText(options[i].c_str(), screenWidth / 2 + 10, screenHeight / 2 - (menuOptionsCount - i) * fontSize + 80, fontSize, GREEN);
            }
            else {
                DrawText(options[i].c_str(), screenWidth / 2, screenHeight / 2 - (menuOptionsCount - i) * fontSize + 80, fontSize, WHITE);
            }
        }
        if (selectedOptionH >= 0 && selectedOptionH < menuOptionsCountH) {
            DrawText("<", screenWidth / 2 - MeasureText(options2[selectedOptionH].c_str(), fontSize) - 100, 350, fontSize, GREEN);
...
void Minando() {
    ...
    const int animationFrames = 30;  // Número de fotogramas de la animación
    const int frameDuration = 15;   // Duración de cada fotograma en milisegundos
    ...
    // SPRITE
    Texture2D spritesheet = LoadTexture("../assets/sprite2.png");
    int spriteWidth = spritesheet.width /14;
    int spriteHeight = spritesheet.height;  // Si los sprites están en una sola columna
    Rectangle sourceRect = { 0, 0, static_cast<float>(spriteWidth), static_cast<float>(spriteHeight) };
    int currentFrame2 = 0;
    float frameTime = 0.0f;
    Vector2 position = { 720, 200 };
    ...
    while (!exitLoading) {
        ...
        DrawText(loadingText.c_str(), screenWidth / 2 - MeasureText(loadingText.c_str(), fontSize) / 2 -35, 600, fontSize, WHITE);
        ...
        DrawText(dots.c_str(), screenWidth / 2 + MeasureText(loadingText.c_str(), fontSize) / 2 -25, 600, fontSize, WHITE);
...
int main() {
    ...
    while (inFile >> monto >> read_emisor  >> receptor  >> dia  >> mes >> year) {
        transaccion t(monto, read_emisor, receptor, dia, mes, year);
