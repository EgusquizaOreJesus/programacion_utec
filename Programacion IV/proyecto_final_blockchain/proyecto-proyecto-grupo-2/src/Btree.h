...
bool BTree<TK>::search(Node<TK>*& node, TK key){
    ...
    for(int i = 0; i < node->count; i++){
        if(comparador::igualdad(node->keys[i],key)){
            return true;
        } else{
            //key < node->keys[i]
            if(comparador::isMayor(key,node->keys[i])){
...
void BTree<TK>::remove(Node<TK>* child, Node<TK>* parent, TK key){
    ...
    bool found = false;
    for(; i < child->count;i++){
        if(comparador::igualdad(key,child->keys[i])){
            found = true;
            break;
        }
    }
    ...
    if (!found & child->leaf){
        ...
        for(; idx<child->count; idx++){
            if(comparador::isMayor(key,child->keys[idx])){
