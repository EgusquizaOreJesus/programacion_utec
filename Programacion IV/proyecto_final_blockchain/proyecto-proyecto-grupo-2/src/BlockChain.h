...
struct block{
    ...
    bool getValido()
    {
        valido_prev = prev_hash.substr(0,4) == "0000";
        valido_current = current_hash.substr(0, 4) == "0000";
        return valido_current && valido_prev;
    }
    void printblock(){
        cout<<"ID :"<<id<<endl;
...
private:
    ...
    BTree_M<Monto>* index_on_monto2 = new BTree_M<Monto>(3);
...
public:
    ...
    void insert_transaction(int index,transaccion& data){
        ...
        block_references[index]->data2[block_references[index]->data2.size()-1].setIdBloque(index);
    ...
    void edit_block(int index_b , int index_t ,const T& dato ,bool emisor = false){
        transaccion data_old = block_references[index_b]->data2[index_t-1];
        Monto prov_old{};
        prov_old.monto=data_old.monto;
        prov_old.index_bloque=index_b;
        index_on_transaction->remove(data_old);
        if (is_same_v<T,double>){
            index_on_monto2->remove(prov_old);
            cout << "UWU" << endl;
        }
        if (!is_same_v<T,double> && emisor){
            index_on_names.remove(data_old.emisor);
        }
        else if (!is_same_v<T,double> && !emisor){
            index_on_names.remove(data_old.receptor);
        }
        if constexpr(is_same_v<double,T>){
            cout << "double";
        }
        ...
        transaccion data = block_references[index_b]->data2[index_t-1];
        cout << "emisor " << data.emisor << endl;
        auto instance_user1 = new Usuario(data.emisor);
        auto instance_user2 = new Usuario(data.receptor);
        if(!usuarios.contains(data.emisor))
        {
            instance_user1->nueva_operacion(index_b);
            usuarios.insert(make_pair(instance_user1->getNombre(),instance_user1));
        }else usuarios[data.emisor]->nueva_operacion(index_b);

        if(!usuarios.contains(data.receptor))
        {

            instance_user2->nueva_operacion(index_b);
            usuarios.insert(make_pair(instance_user2->getNombre(),instance_user2));
        }else usuarios[data.receptor]->nueva_operacion(index_b);
        ...
        id_last_edited = 1;
        index_on_transaction->insert(data);
        cout <<"search trans: " <<boolalpha << index_on_transaction->search(data)  <<endl;
        Monto prov{};
        prov.monto=data.monto;
        prov.index_bloque=index_b;
        index_on_monto.insert(prov);
        index_on_monto2->insert(prov);
...
//CRITERIOS DE BUSQUEDA -->Igual a X
    ...
    vector<transaccion> search(const string& data,eleccion a){
        ...
        for (int i = 1; i <= id; ++i) {
            for (auto& trans: block_references[i]->data2) {
                if (a == receptor){
...
//CRITERIOS DE BUSQUEDA Igual -->Entre X y Y
    ...
    {   Monto inicial , final;
        ...
        return index_on_monto2->rango_search(inicial , final );
    }
...
//CRITERIOS DE BUSQUEDA  -->Máximo valor de
    ...
    {  vector<int> ids;
        for(auto t:filtrados){
            auto iter = std::find(ids.begin(), ids.end(), t.index_bloque);
            if (iter == ids.end()){
